    ...
    if (!args[0]) {
        return message.channel.send('```y!t-remove <id> <raison>```')
                ...
                if (message.member.roles.cache.get(ticketsaccess)) {
                    ...
                    const user = await client.users.fetch(`${client.dbTickets.get(`User_${message.channel.id}`)}`);
...
exports.help = {
    ...
    usage: "t-remove <id utilisateur> <raison>",
    example: ["t-remove 782667133716791316 Pas concerné par la suite de ce ticket."]
}