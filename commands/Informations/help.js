  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `Envoyez \`${prefix}help [commande]\` pour obtenir plus d'informations !`
        )
      ...
      if (!command) {
        return message.channel.send(`${client.no} ➜ Impossible de retrouver une commande nommée \`${prefix}${args[0]}\``);
      }
