...
class Delwarn extends Command {
    ...
    async run(client, message, args) {
        if (!parseInt(args[1])) return message.reply(`**${client.no} ➜ Veuillez entrer un identifiant valide.**`)
        const db = await warns.findOne({ wrnID: Number(args[1]) });
        ...
        await warns.findOneAndDelete({ warns: Number(args[1]) });
